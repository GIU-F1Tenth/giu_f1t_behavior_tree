/**
 * @file OptimizedPathNode.cpp
 * @brief Implementation of OptimizedPathNode
 */

#include "OptimizedPathNode.hpp"

OptimizedPathNode::OptimizedPathNode(const std::string &name, const BT::NodeConfiguration &config)
    : SyncActionNode(name, config)
{
    ...
    node_->declare_parameter("path_decisions.optimized_path", "csv_race_path");
    ...
    node_->get_parameter("path_decisions.optimized_path", optimized_decision_message_);
    ...
    RCLCPP_INFO(node_->get_logger(), "OptimizedPathNode initialized");
}

BT::NodeStatus OptimizedPathNode::tick()
{
    // Publish decision to use optimized path
    std_msgs::msg::String decision_msg;
    decision_msg.data = optimized_decision_message_;
    ...
    RCLCPP_DEBUG(node_->get_logger(), "Published optimized path decision and disabled gap follower");
